...
export class PhoneVerificationComponent implements AfterViewInit {
  @Input() phoneNumber: string = '+1 650-555-3433';

  @Input() navigateUrlAfterVerification: string = '/auth/verify-phone';

  @Input() navigateBackUrl: string = '/auth';
  ...
  public recaptchaChecked: boolean = false;
  ...
  private createRecaptchaVerifier() {
    return new RecaptchaVerifier(
      ...
      'recaptcha-container',
      { size: 'compact', callback: () => (this.recaptchaChecked = true) }
    );
  }

  public sendVerificationCode() {
    this.confirmationResult$ = this.authService.sendOtpOnPhone(
      this.phoneNumber,
  ...
  public onVerify() {
    this.authService
      .verifyPhone(
        ...
        this.phoneVerificationForm.value.code!
      )
      .pipe(take(1))
      .subscribe({
        next: (credentials) => {
